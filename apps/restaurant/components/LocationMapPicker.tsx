...
type ResolvedProvider = { kind: "streetmaps" } | { kind: "google"; apiKey: string } | { kind: "mapbox"; accessToken: string };
 ...
 * default, or Google/Mapbox once an admin has saved a working key and
 * switched to it. Always falls back to Streetmaps if the chosen
 * provider's key is missing, so this can never render a broken map.
 * Every call site imports this exact component/prop shape — the
 * underlying provider swap is invisible to them. */
export function LocationMapPicker(props: MapPickerProps) {
  ...
  if (provider.kind === "google") return <GoogleMapPicker {...props} apiKey={provider.apiKey} />;
  if (provider.kind === "mapbox") return <MapboxMapPicker {...props} accessToken={provider.accessToken} />;
