...
type Props = {
   ...
   * "full" fills its parent's height edge-to-edge, for a dedicated chat screen. */
...
export function OrderChat({ orderId, variant = "embedded" }: Props) {
  ...
  const inputPill = (
    <div className="relative w-full">
      ...
      <button
        ...
        className="absolute right-1.5 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full bg-gold text-ink shadow-[0_4px_12px_rgba(201,162,39,0.35)] transition-opacity disabled:opacity-40"
  ...
  if (variant === "full") {
    return (
      <div className="flex h-full flex-col">
        ...
        <form
          onSubmit={send}
          className="flex h-[100px] shrink-0 items-center border-t border-[var(--border-faint)] bg-cream px-4"
        >
          {inputPill}
        </form>
      </div>
  ...
  return (
    <section className="space-y-2.5">
      ...
      <form onSubmit={send}>{inputPill}</form>
    </section>
  );
}
