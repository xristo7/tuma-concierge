...
/** Plays back the raw voice note a customer attached to an order — spoken
 * context a typed list can miss (units, brand, exactly where in the shop). */
export function VoiceNotePlayer({ orderId }: { orderId: string }) {
  ...
  return (
    ...
    >
      ...
      {status === "error" ? "Couldn't load voice note" : status === "playing" ? "Playing…" : "Play customer's voice note"}
    </button>
  );
}
